...
class PlayerCollection:
    def __init__(self, win, top,player):
        ...
        cs=[(0,1),(0,2),(0,3),(1,2),(1,3),(2,3),(0,0),(1,1),(2,2),(3,3)]
        for i in range(5):
    ...
    def humanSetup(self):
        ...
        self.switchB =Button(self.win,Point(self.x-35,self.win.getHeight()-100),12,10,'white','Switch Tile \nOrientation')
        self.switchB.offSet(0,25)
        self.switchB.deactivate()
        self.placeB=Button(self.win,Point(self.x+35,self.win.getHeight()-100),12,10,'white','Place Tile')
        self.placeB.offSet(0,25)
        self.placeB.deactivate()
        y = self.top.getY()+85
        for tile in self.set:
            button=Button(self.win, Point(self.x,y),10,8,'white','Choose')
            button.offSet(0,10)
            button.activate()
